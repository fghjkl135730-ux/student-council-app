...
export default function NewMeetingEntryScreen() {
  ...
  return (
    <ScreenContainer edges={['top', 'left', 'right']}>
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.foreground }]}>새 회의록</Text>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <View style={styles.section}>
          ...
          <TouchableOpacity
            style={[styles.summarizeBtn, { backgroundColor: isSummarizing ? colors.muted : colors.primary }]}
          ...
          <View style={styles.section}>
            ...
            {summary.keyPoints.length > 0 && (
              <View style={[styles.summaryBlock, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                <Text style={[styles.summaryBlockTitle, { color: colors.foreground }]}>📌 핵심 내용</Text>
            ...
            {summary.decisions.length > 0 && (
              <View style={[styles.summaryBlock, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                <Text style={[styles.summaryBlockTitle, { color: colors.foreground }]}>✅ 결정사항</Text>
            ...
            {summary.actionItems.length > 0 && (
              <View style={[styles.summaryBlock, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                <Text style={[styles.summaryBlockTitle, { color: colors.foreground }]}>🎯 액션 아이템</Text>
                    ...
                    <View style={styles.actionMeta}>
                      {item.assignee && (
                        <View style={[styles.assigneeBadge, { backgroundColor: '#8B5CF6' + '18' }]}>
                          <Text style={[styles.assigneeText, { color: '#8B5CF6' }]}>담당: {item.assignee}</Text>
                        </View>
                      )}
                      {item.deadline && (
...
const styles = StyleSheet.create({
  ...
  inputLabel: { fontSize: 14, width: 36 },
  textInput: { flex: 1, fontSize: 15, fontWeight: '500' },
  inputDivider: { height: 1, marginLeft: 16 },
  textAreaContainer: { borderRadius: 12, borderWidth: 1, padding: 12 },
  textArea: { fontSize: 14, lineHeight: 22, minHeight: 200 },
  summarizeBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginTop: 12, paddingVertical: 14, borderRadius: 12, gap: 8 },
  summarizeBtnText: { color: '#fff', fontSize: 15, fontWeight: '700' },
  summaryHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 },
  summaryBadge: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 10, paddingVertical: 5, borderRadius: 8, gap: 5 },
  summaryBadgeText: { fontSize: 13, fontWeight: '700' },
  summaryBlock: { borderRadius: 12, borderWidth: 1, padding: 14, marginBottom: 10, gap: 8 },
  summaryBlockTitle: { fontSize: 14, fontWeight: '700', marginBottom: 4 },
  summaryItem: { flexDirection: 'row', alignItems: 'flex-start', gap: 8 },
  summaryDot: { width: 6, height: 6, borderRadius: 3, marginTop: 6 },
  summaryItemText: { fontSize: 13, lineHeight: 20, flex: 1 },
  actionItem: { borderTopWidth: 1, paddingTop: 10, gap: 6 },
  actionTask: { fontSize: 13, fontWeight: '600', lineHeight: 18 },
